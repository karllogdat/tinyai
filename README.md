...
- `c/`: contains the C implementation of the TinyAI. Depends on the `transition_table.h` and
   `transition_table.c` generated by the C++ version.
- `cpp/`: contains the C++ implementation of TinyAI. Contains the lexer generator logic that runs
   the C version.
- `examples/`: contains sample source files of TinyAI.
